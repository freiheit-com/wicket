#!/bin/sh
echo -n "Promoting release 7.0.0
...
git push origin wicket-7.0.0

# promote the source distribution by moving it from the staging area to the release area

svn mv https://dist.apache.org/repos/dist/dev/wicket/7.0.0 https://dist.apache.org/repos/dist/release/wicket -m "Upload release to the mirrors"

mvn org.sonatype.plugins:nexus-staging-maven-plugin:LATEST:rc-release -DstagingRepositoryId=orgapachewicket-1049 -DnexusUrl=https://repository.apache.org -DserverId=apache.releases.https -Ddescription="Release vote has passed"

# Renumber the next development iteration 7.1.0-SNAPSHOT:
...
./wicket-experimental/wicket-atmosphere/pom.xml
...
./wicket-native-websocket/wicket-native-websocket-jetty/pom.xml
./wicket-native-websocket/wicket-native-websocket-jetty9/pom.xml
./wicket-native-websocket/wicket-native-websocket-tomcat/pom.xml
...
echo "Remove the previous version of Wicket using this command:
  ...
  svn rm https://dist.apache.org/repos/dist/release/wicket/7.0.0-SNAPSHOT -m "Remove previous version from mirrors"
