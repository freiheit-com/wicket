#!/bin/sh
echo -n "Reverting release 7.0.0
...
# clean up local repository
git checkout master
git branch -D build/wicket-7.0.0
git tag -d wicket-7.0.0

# clean up staging repository
git push staging --delete refs/heads/build/wicket-7.0.0
git push staging --delete wicket-7.0.0

# clean up staging dist area
svn rm https://dist.apache.org/repos/dist/dev/wicket/7.0.0 -m "Release vote has failed"

# clean up staging maven repository
mvn org.sonatype.plugins:nexus-staging-maven-plugin:LATEST:rc-drop -DstagingRepositoryId=orgapachewicket-1049 -DnexusUrl=https://repository.apache.org -DserverId=apache.releases.https -Ddescription="Release vote has failed"
